class n{constructor(e,t){this.shop=e,this.tower=t,this.isOpen=!1,this.container=null,this.boundUpdate=()=>this.render()}show(){this.isOpen||(this.isOpen=!0,this.container=document.createElement("div"),this.container.className="tower-shop-overlay",document.body.appendChild(this.container),document.addEventListener("towerStateChanged",this.boundUpdate),this.render())}hide(){this.isOpen&&(document.removeEventListener("towerStateChanged",this.boundUpdate),this.container?.parentNode&&this.container.parentNode.removeChild(this.container),this.container=null,this.isOpen=!1)}render(){if(!this.container)return;const e=this.tower.getState(),t=this.shop.getAllItems(e.shadowShards);this.container.firstElementChild||(this.container.innerHTML=`
    ...
    `}formatEffect(e){if(e.level===0)return"нет";const t=e.effectValue;switch(e.stat){case"damage_percent":return`+${t}% урона`;case"hp_percent":return`+${t}% HP`;case"regen_per_hit":return`+${t} HP за удар`;case"crit_chance":return`${t}% шанс крита`;case"crit_damage":return`+${t}% крит. урон`;case"shard_bonus":return`+${t}% осколков`;case"gold_bonus":return`+${t}% теней`;case"enemy_slow":return`-${t}% скорость врага`;case"checkpoint_heal":return`+${t}% HP на чекпоинте`;case"auto_damage":return`${t}% авто-урон/5с`;default:return`${t}`}}getIcon(e){return{tower_damage:"sword",tower_hp:"heart",tower_regen:"tint",tower_crit_chance:"eye",tower_crit_damage:"bolt",tower_shard_bonus:"gem",tower_gold_bonus:"coins",tower_enemy_slow:"snowflake",tower_checkpoint_heal:"medkit",tower_auto_damage:"ghost"}[e]||"cube"}bindActions(){const e=this.container.querySelector("#close-tower-shop"),t=this.container.querySelector("#close-tower-shop-btn");e?.addEventListener("click",()=>this.hide()),t?.addEventListener("click",()=>this.hide())}bindBuyActions(){this.container.querySelectorAll(".tower-shop-buy-btn").forEach(t=>{t.addEventListener("click",s=>{const o=s.currentTarget.dataset.id;this.handleBuy(o)})})}handleBuy(e){const t=this.tower.getState(),s=this.shop.buyUpgrade(e,t.shadowShards);s.success&&(this.tower.shadowShards=s.newShards,this.tower.triggerUpdate(),this.render())}}export{n as TowerShopModal};
